...
function Breadcrumbs(jarry) {
    /// <summary> 面包屑导航 </summary>
    /// <param name=" j " type="String"> </param>
    ...
    for (var i = 0; i < jarry.length; i++) {
        ...
        w.push("<a class=\"a_mbx_index\" href='" + bUrl + "'>" + bName + "</a>");

    }


    //  $("#div_Path").before("<div class='clr' style='height:3px;' ></div>");
    $("#sp_path1").html(w.join(""));
    $("#sp_path1").show();
}
...
function tiaozhuan(url, NewWindow) {
...
function ToPage(sObj, PageId) {
    ...
    if (ToPageNum < 1) {
        ...
        eval("BeforePageBtn(pageIndex,this); " + sObj + "(ToPageNum);");
    }
}
//#endregion
...
function OpImg() {
    ...
    var ifm = '<iframe id="ifm_SaveImg" src="/PubUI/CropPicture2.aspx frameborder="0" style="height:430PX; width:650px; border:none;" border=0  />';
...
function OpImgCutting(wh) {
    ...
    /// <param name=" wh " type="String">  1|1 比例 </param>
    var ifm = '<iframe id="ifm_SaveImg" src="/PubUI/FlashCutImg/demo.html?wh=' + wh + '" frameborder="0" style="height:530PX; width:830px; border:none;" border=0  />';
...
function PopWindow(op, width, height) {
    ...
    w.push("<div  id='" + popId + "' class='div_pop1' style='width:" + width + "px;   margin:-" + ConvertToInt(height / 2) + "px 0 0  -" + ConvertToInt(width / 2) + "px;' >");
    w.push("<dl class='dl_pop1' >");
    w.push("<dt class='dt_pop1' style='width:" + width + "'>");
    w.push("<span class='sp_pop_hd'>");
    w.push(op.title);
    w.push("</span>");
    w.push("<a class='a_guanbi' onclick='ClearPopWindow()' >关闭</a>");
    w.push("</dt>");
