...
namespace Common
{
    ...
    public class Uploader2
    {
      ...
      * @return Hashtable
      */
        public Hashtable upFile(HttpContext cxt, string pathbase, string[] filetype, int size, bool isImg)
        {
            string Watermark = "";
            try
            {
                HttpUtility.UrlDecode(Watermark = cxt.Request.Params["Watermark"]);
                if (Watermark == null)
                {
                    Watermark = "";
                }
            ...
            try
            {
                ...
                if (state == "SUCCESS")
                {
                    ...
                    var FilePath = (uploadpath + filename).ToLower();
                    uploadFile.SaveAs(FilePath);


                    if (isImg)
                    {
                        var FilePath200 = FilePath.Replace("\\upload\\", "\\upload200\\");
                        Common.ImgHelper.MakeThumNail(FilePath, FilePath200, 200, 200, "W");
                        try
                        {
                            if (Watermark.Trim() != "")
                            {
                                Common.ImageClass.ImageWatermark(URL, Watermark, "C");
                            }
                        }
                        catch (Exception)
                        {
